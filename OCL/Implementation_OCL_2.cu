...
if(argc < 6)
	...
	int retRSS = DoRSSOCLOperation(memObj_data[0], rss, dim, sizeCfl, slice, numSlice, context, commandQueue, program, numThread);
	if(retRSS == 1)
	{
	...
	cout << "Timing " << std::to_string(GPUTimer_getspec*1000) << " " << std::to_string(GPUTimer_zeroFill*1000) << " " << std::to_string(timesRecons[0]*1000) << " " << std::to_string(timesRecons[1]*1000)<<" " << std::to_string(timesRecons[2]*1000) << " " << std::to_string(timesRecons[3]*1000) << " " <<std::to_string(GPUTimer_RSSRaw*1000) << " " << std::to_string(GPUTimer*1000)<< endl;
	cout << "==========================================================================================================" << endl;
